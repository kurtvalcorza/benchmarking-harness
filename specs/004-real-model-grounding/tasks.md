...
- [ ] T111 [P] [US2] Attribution **canonicalization** guard: `person` attribution + `label_map {person: pedestrian}` → pointing-game **hit** inside the pedestrian box; without canon → miss — `backend/tests/unit/test_grounding_canon.py`
...
- [ ] T141 [US4] Bounded explain: stop after `grounding_min_samples` targets; `log()` the cap (images explained / targets reached) — no silent truncation (FR-309)
