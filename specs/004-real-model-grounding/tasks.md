...
- [ ] T121 [US1] `engine/adapters/pytorch_adapter.py` `_predict_det`: **explain** step appends `{label, point, energy_inside}` per detection; `point` in original-image pixels (FR-301/304)
- [ ] T121a [US4] **Explain seam** (FR-306a/308, review #1/#2): thread `explain: bool = False` through `engine/sandbox/runner.py::run_inference` → the adapter predict path; time the clean forward vs the extractor separately → `JobResult.timing` gains `predict_s`/`explain_s`. Tier 1 (`tier1_capability.py:39`) + Tier 2 (`tier2_stress.py:82`) call with the default (no attribution/no cost); Tier 3 (`tier3_ops.py:62`) passes `explain=True`. `explain=False` keeps timing byte-for-byte
