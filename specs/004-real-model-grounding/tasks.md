...
- [ ] T117 [P] [US4] **Remote-runner leg** test (FR-306a, 3rd-round review — the silent gap): a `POST /run` to `backend/runner/main.py` with `explain=True` in the body MUST propagate to attribution (not be dropped by Pydantic); assert `RunRequest` carries `explain` and the handler forwards it — `backend/tests/contract/test_runner_service.py` (or the existing runner-boundary test). T113/T116 exercise `run_inference` directly and do NOT cover this HTTP round-trip
...
- [ ] T121a [US4] **Explain seam** (FR-306a/308, reviews #1/#2 + 2nd/3rd round): `InferenceAdapter` is a `Protocol`, so `explain()` is OPTIONAL — `job.py` invokes via `getattr(adapter, "explain", None)` (pytorch + stub implement it; ONNX/others skip). Thread `explain: bool = False` through **both** legs of `engine/sandbox/runner.py::run_inference`: (a) the `spec` dict → `engine/sandbox/job.py::run(spec)`, which after the timed clean `predict()` (`job.py:105-110`) runs a **separately-timed** `explain()` → `JobResult.timing` gains `predict_s`/`explain_s`; and (b) the T073 HTTP round-trip — **both sides**: `app/services/runner_client.py::run_remote()` sends `explain` in the body **and** `backend/runner/main.py` `RunRequest` (`:49-53`) gains `explain: bool = False` + `POST /run` (`:76-90`) forwards it (else it silently no-ops under `HARNESS_RUNNER_URL` — Pydantic drops the unknown field). Tier 1 (`:39`) + Tier 2 (`:82`) default off; Tier 3 (`:62`) passes `explain=True`. `explain=False` keeps every leg byte-for-byte
