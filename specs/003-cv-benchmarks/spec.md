...
- **FR-214** The Tier-2 per-class safety-floor mechanism, today recall-only (`recall_floors` + `metrics['per_class_recall']`), MUST generalize so a segmentation golden set can declare per-class **IoU** floors that are checked against per-class IoU. Registration schema/storage and the Tier-2 floor check MUST both handle the segmentation metric; a segmentation golden set with IoU floors MUST NOT be rejected by a recall-only schema or flag every class as missing recall. The generalization MUST also cover the **recall-named downstream verdict paths** (the flag trigger `safety_critical_recall_below_floor` / `FlagInput.safety_recall_breach` and the Model Card wording), so an IoU-floor breach is reported as an IoU breach, not a "recall" one.
- **FR-215** `metrics.canonicalize()` (run on every prediction set before Tier-1/2 scoring) MUST preserve the new segmentation mask channel. It currently rebuilds `Prediction` with only the detection/classification fields; if masks are dropped there, every segmentation score is empty regardless of the model.
- **FR-216** The coverage/validation layer MUST reject malformed mask payloads with typed errors (bad RLE counts, out-of-range polygon coordinates, mask dimensions that do not match the image), in addition to the existing missing/duplicate/extra accounting — a malformed mask must never silently score as empty or crash the scorer.
- **FR-217** A **deterministic instance→semantic reduction policy** MUST be defined and documented, because the Ultralytics `-seg` adapter emits per-instance masks (FR-204) while the metric is semantic per-class mIoU (FR-202): same-class instance union, cross-class overlap resolution (e.g. higher-confidence instance wins the pixel), and a fixed ordering, so the reduction is reproducible (SC-004).
- **FR-218** Mask predictions MUST be persisted as **content-addressed evidence** (FR-208). Today Tier-1/2 persistence records only metadata (sandbox timing/descriptor) and does not store raw predictions; segmentation MUST stage/digest/publish the masks so a verdict's mask evidence is resolvable and reproducible.
- **FR-219** Segmentation Golden Test Set registration MUST **reject a dataset that lacks valid masks** (the current dataset validator only requires `label` + optional `bbox`), so a detection/classification-shaped dataset cannot be registered as a segmentation benchmark.
